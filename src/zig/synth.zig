...
export fn sfxBuffer(u8Array: [*]u8, u8ArrayLength: usize, sampleRate: u32) void {
    const hz = 256;
    const samples_per_wave = sampleRate / hz; // 32 desired;
    for (u8Array[0..u8ArrayLength]) |_, i| {
        u8Array[i] = @floatToInt(u8, @sin(@intToFloat(f32, i % samples_per_wave) / @intToFloat(f32, samples_per_wave) * 2 * pi) * 127.5 + 127.5);
    }
}
