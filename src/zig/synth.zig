...
export fn sfxBuffer(u8Array: [*]u8, u8ArrayLength: usize, sampleRate: u32) void {
    ...
    while (i < u8ArrayLength) : (i += samples_per_wave) {
        ...
        while (j < period_or_end) : (j += 1) {
            u8Array[j] = @floatToInt(u8, @sin(@intToFloat(f32, j % samples_per_wave) / @intToFloat(f32, samples_per_wave) * 2 * pi) * 127.5 + 127.5);
        }
        periods_at_note += 1;
        if (periods_at_note >= period_per_note) {
            periods_at_note = 0;
            hz_index += 1;
            if (hz_index >= NoteHz.len) {
                hz_index = 30;
            }
            hz = NoteHz[hz_index];
